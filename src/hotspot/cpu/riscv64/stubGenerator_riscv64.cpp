...
#define inc_counter_np(counter) \
  ...
  address generate_string_indexof_linear(bool needle_isL, bool haystack_isL)
  {
    ...
    __ andr(ch2, match_mask, trailing_zero); // clear useless bits and check
    __ beqz(ch2, NOMATCH);

    __ bind(L_SMALL_HAS_ZERO_LOOP);
    __ ctz_bit(trailing_zero, match_mask, ch2, tmp); // count trailing zeros
    __ mv(ch2, wordSize / haystack_chr_size);
    ...
    __ ctz_bit(trailing_zero, match_mask, tmp, ch2);
    ...
    __ ctz_bit(trailing_zero, match_mask, needle_len, ch2); // find next "first" char index
    __ add(haystack, haystack, haystack_chr_size);
